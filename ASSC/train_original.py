...
def get_model():
	model = Sequential()
	model.add(Conv2D(60, (1,25), activation='relu', input_shape=(3,9000,1),padding='same'))
	model.add(BatchNormalization())
	model.add(MaxPooling2D(pool_size=(1,5)))
	model.add(Conv2D(80,(1,20), activation='relu',padding='same'))
	model.add(BatchNormalization())
	model.add(MaxPooling2D(pool_size=(1,5)))
...
def DFT(data):
	f,t,z=signal.stft(data,fs=100,window='hamming',nperseg=540,noverlap=0,nfft=999)
...
def WT(data):
	...
	xnew = np.arange(0,len(sig[0])-1,(len(sig[0])-1)/9000)
	ynew = f(xnew)
	return ynew
...
def main():
	...
	plt.title(train_y[int(argv[1])])
	plt.plot(train_data[int(argv[1])])
	plt.show()
	input()
	...
	model = get_model()
	opt = Adam(lr=0.002, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0, clipvalue=3) #initial:lr=0.001
	...
	model.fit(train_x, train_y, epochs=100, shuffle=True, batch_size=128, verbose=1, validation_split=0.05)
